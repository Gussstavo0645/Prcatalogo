    ...
    <form method="GET" action="{{ route('admin.premios.index') }}" class="card card-body mb-4">
        <div class="row align-items-end">
            <div class="col-md-4">
                ...
                <input type="text" name="mes" value="{{ $mes }}" class="form-control" placeholder="06/2026">
            </div>
            ...
            <table class="table table-hover align-middle mb-0">
                ...
                <tbody>
                    @forelse($premios as $premio)
                        <tr>
                            <td class="fw-bold">{{ $premio->Codigo }}</td>
                            <td>{{ $premio->Descripcion }}</td>
                            <td>{{ $premio->Mes }}</td>
                            <td>{{ $premio->Codprod }}</td>
                            <td>{{ $premio->Cantidad }}</td>
                            <td>Q{{ number_format($premio->Desde, 2) }}</td>
                            <td>Q{{ number_format($premio->Hasta, 2) }}</td>
                            <td>
                                <a href="{{ route('admin.premios.edit', [$premio->Codigo, $premio->Mes]) }}"
                                   class="btn btn-sm btn-warning">
                                    Editar
                                </a>

                                <form action="{{ route('admin.premios.destroy', [$premio->Codigo, $premio->Mes]) }}"
                                      method="POST"
                                      class="d-inline"
